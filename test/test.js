...
const ZipsceneApiClient = require('../lib/zipscene-api-client');
...
describe('ZipsceneApiClient', () => {
	...
	it('should fail to create client with no credentials', () => {
		...
		expect(() => { return new ZipsceneApiClient(opts); }).to.throw(XError);
	});

	it('should authenticate with given credentials', () => {
		let client = new ZipsceneApiClient(DEFAULT_CONFIG);
	...
	it('should fail to authenticate with invalid credentials', () => {
		let client = new ZipsceneApiClient(objtools.merge(
			{},
			DEFAULT_CONFIG,
