...
XError.registerErrorCode('invalid_object', { message: 'Received invalid line from request stream' });
/**
...
class JsonRPCApiClient {
	...
	constructor(settings) {
		...
		// Preserve access token if it's provided
		if (this.accessToken) {
			this.originalAccessToken = this.accessToken;
		}
	...
	*/
	authenticate() {
		...
		} else if (this.originalAccessToken) {
			this.accessToken = this.originalAccessToken;
			this.authPromise = Promise.resolve();
	...
	*/
	request(method, params, id = null, opts = {}) {
		...
		.then(({ error, result }) => {
			if (error) {
				let responseError = new XError(error.code, error.message, error.data);
				responseError._isRemote = true;
				throw responseError;
			}
			return result;
		});
