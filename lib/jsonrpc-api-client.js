	...
	*/
	requestStream(method, params, id) {
		...
		let authenticateAndMakeHeaders = () => {
			this.authPromise = this.authenticate();
			return this.authPromise
				.then(() => this.createBearerHeader(this.accessToken))
				.catch((error) => {
		...
		let setUpStreamEndListener = (stream) => {
			stream.on('finish', () => {
				let success = lastSeenDataObj.success;
				let error = lastSeenDataObj.error;
				if (success) {
					return passThrough.end();
				} else if (error) {
					return passThrough.emit('error', new XError(
						error.code,
						error.message
					));
				} else {
					...
					return stream
						...
						.through((entry) => {
							...
							if (parsedEntry.error) {
								...
								return writeToPassThrough(parsedEntry);
							}
						})
						.intoPromise()
						.then(() => {
