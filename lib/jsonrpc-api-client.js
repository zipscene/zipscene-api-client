...
class JsonRPCApiClient {
	...
	*/
	...
	/**
	* This checks for an authorization error, if it is it will throw it.
	*
	* @method _handleError
	* @private
	...
	* This method will __NOT__ handle errors that occur after the stream has begun
	*
	* @method request
	...
	* @return {Promise} - resolves with a response stream strings where the last object is success: true
	* @since v0.0.1
	*/
			...
			return this.authenticate()
				...
				.then((headers) => {
					return new Promise((resolve, reject) => {
						...
						zstreams(request({ uri, headers, json, method: 'post' }))
							.through(function(data) {
								let tmpData = JSON.parse(data);
								if (tmpData.error && !resolved) {
									let { code } = tmpData;
									if (code === 'bad_access_token' || code === 'token_expired') {
