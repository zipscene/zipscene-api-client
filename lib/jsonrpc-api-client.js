	...
	*/
	export(profileName, params = {}) {
		...
		let setUpStreamEndListener = (stream) => {
			stream.on('finish', () => {
				let success = _.get(lastSeenDataObj, 'success');
				let error = _.get(lastSeenDataObj, 'error');

				if (success) { return passThrough.end(); }

				if (error) { return passThrough.emit('error', lastSeenDataObj.error); }

				if (hasWritten && !success) {
