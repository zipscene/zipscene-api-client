...
async function getRPCClient(service = 'dmp', argv = {}, config = null) {
	...
	let otherOptions = {};
	if (process.env.ZIPSCENE_API_CLIENT_LOG_REQUESTS) {
		otherOptions.logRequests = true;
	}

	let options = objtools.merge({}, serverOptions, authOptions, otherOptions);
...
async function getDataClient(profileType, argv = {}, config = null, profiles = false) {
	if (!config) config = projectConfig;
